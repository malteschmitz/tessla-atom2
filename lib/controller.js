...
export default class Controller {
  ...
  onPatchAssembly({ onSuccess, onError }) {
    ...
    this.viewManager.views.logView.addEntry(MESSAGE_TYPE.CMD, command);
    ...
    this.runningProcess.on('close', () => {
      ...
      if (errors.length === 0) {
        // first log message
        this.viewManager.views.logView.addEntry(MESSAGE_TYPE.MSG, 'Successfully patched Assembly of this project');
  ...
  onBuildCCode({ buildAssembly, onSuccess, onError }) {
    ...
    this.runningProcess.on('close', () => {
      ...
      if (errors.length === 0) {
        // first log message
        this.viewManager.views.logView.addEntry(MESSAGE_TYPE.MSG, 'Successfully compiled all C files of this project');
        ...
        // first log message
        this.viewManager.views.logView.addEntry(MESSAGE_TYPE.MSG, 'An error occurred while compiling all C files of this project');
  ...
  onRunPatchedBinary({ onSuccess, onError }) {
    ...
    this.runningProcess.stdout.on('data', (data) => {
      // log output
      this.viewManager.console.addEntry(data.toString());
      // increase counter
  ...
  onRunBinary({ onSuccess, onError }) {
    ...
    this.viewManager.views.logView.addEntry(MESSAGE_TYPE.CMD, binary);
    ...
    this.runningProcess.on('close', (code, signal) => {
      ...
      if (errors.length === 0) {
        ...
        // first log message
        this.viewManager.views.logView.addEntry(MESSAGE_TYPE.MSG, 'An error occurred while running the C binary of this project');
  ...
  checkDockerContainer() {
    ...
    if (id.length === '') {
      ...
      this.viewManager.views.logView.addEntry(MESSAGE_TYPE.CMD, `docker ${args.join(' ')}`);
    }
  }
