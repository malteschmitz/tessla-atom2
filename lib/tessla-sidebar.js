...
export default class TeSSLaSidebar {
  ...
  onMouseMove(event) {
    ...
    if (this.mouseDownRowResizer) {
      ...
      var functionsViewHeight = this.$functionsView.outerHeight()
  ...
  onUpdate(activeProject) {
    ...
    var cFiles          = activeProject.cFiles      ? activeProject.cFiles      : []
    var tesslaFiles     = activeProject.tesslaFiles ? activeProject.tesslaFiles : []
    ...
    cFiles.forEach((file) => {
      ...
      var functions = TeSSLaFileManager.collectCFunctionsFromSourceFile({
        sourceFile:  file,
  ...
  onFormatOutput({output, startTime}) {
    ...
    for (var key in formatted) {
      ...
      var $counter = $('<span class="cnt-values">#' + formatted[key].length + '</span>')
      var $entry   = $('<li>')
      var $sublist = $('<ul>')
