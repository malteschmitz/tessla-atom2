...
export function activate(state) {
  ...
  dependencies.install('tessla').then(() => {
    ...
    const args = [
      'run', '--volume', containerDir + ':/tessla', '-w', '/tessla', '-tid', '--name', 'tessla', 'tessla', 'sh'
    ]
    var startDocker = childProcess.spawn('docker', args)
    ...
    startDocker.on('close', () => {
      ...
      emitter.emit('add-log-text', '[CMD] docker ' + args.join(' '))
