...
export default {
  ...
  activate(state) {
    ...
    if (!atom.config.get('tessla.alreadySetUpDockerContainer')) {
      ...
      Downloader.download({
        ...
        // path: path.join(os.tmpdir(), 'tessla-imdea-snapshot.jar'),
        filePath: path.join(this.imageDir, 'tessla-docker.zip'),
        callback: () => {
          // load docker image
          // childProcess.spawnSync('docker', ['rmi', 'tessla']);
          // childProcess.spawnSync('docker', ['load', '-i', path.join(this.imageDir, 'tessla-docker.zip')]);
        },
      });
    }
    ...
    if (!fs.existsSync(this.containerDir)) {
      // create container directory
      fs.mkdirSync(this.containerDir);
    ...
    this.messageQueue.push({type: MESSAGE_TYPE.CMD, msg: `docker ${dockerArgs.join(' ')}`});
  ...
  toggle() {
    ...
    //activeItem.focus();
    // atom.workspace.open(activeItem.getURI());
  },
  ...
  consumeToolBar(getToolBar) {
    ...
    this.toolBarButtons.showLog = toolBar.addButton({
      ...
      tooltip: 'Resets all view components',
      iconset: 'fa',
    });
