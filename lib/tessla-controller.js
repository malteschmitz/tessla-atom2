...
export default class TeSSLaController {
  ...
  createZlogFile() {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      var traceFile = path.join(outputDir, 'instrumented_' + binName + '.trace')
      rules += 'variable_values_cat.DEBUG "' + traceFile + '"; variable_values\n'
      rules += 'function_calls_cat.DEBUG "' + traceFile + '"; function_calls\n'
    }
  ...
  onPatchAssembly({onSuccess, onError}) {
    ...
    if (!atom.config.get('tessla.useDocker')) {
      ...
      if ( !fs.existsSync(libInstrumentFunctions) ) {
        this.viewMgr.showNoLibInstrumentFunctionsSONotification()
        return
      }

      // get clang path
      var clangPath = path.dirname(atom.config.get('tessla.clangPath'))
      var opt       = path.join(clangPath, 'opt')

      // if there are some tools missing show notification to the user and stop
      // further execution
      if ( !fs.existsSync(opt) ) {
        this.viewMgr.showNoValidOptNotification()
        return
      }
    }
    ...
    // get vars for paths
    var projPath  = this.activeProject.projPath
    var outputDir = this.activeProject.outputDir
    var binName   = this.activeProject.binName
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      args = [
        ...
        path.join(outputDir, binName + '.bc')
      ]
      args = args.concat(instrumentArgs)
      args = args.concat(['>', path.join(outputDir, 'instrumented_' + binName + '.bc')])
      // start process
      this.runningProcess = childProcess.spawn(command, args, {shell: true})
    }
  ...
  onBuildTeSSLa({onSuccess, onError}) {
    ...
    if (!atom.config.get('tessla.useDocker')) {
      ...
      if (!fs.existsSync(tessla) ) {
        this.viewMgr.showNoValidTeSSLaCompilerNotification()
        return
      }
    }
    ...
    var file = this.activeProject.tesslaFiles[0]
  ...
  onBuildAssembly({onSuccess, onError}) {
    ...
    if (!atom.config.get('tessla.useDocker')) {
      ...
      if ( !fs.existsSync(clang) ) {
        this.viewMgr.showNoValidClangNotification()
        return
      }

      // get instrument functions library path
      var libInstrumentFunctions = atom.config.get('tessla.libInstrumentFunctions')
      var libDir = path.dirname(libInstrumentFunctions)
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      command = clang
      args = [
        path.join(this.activeProject.outputDir, 'instrumented_' + this.activeProject.binName + '.bc'),
        '-o',
        path.join(this.activeProject.outputDir, 'instrumented_' + this.activeProject.binName),
        '-lzlog',
        '-lpthread',
        '-L/usr/local/lib',
        '-L' + libDir,
  ...
  onBuildCCode({buildAssembly, onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      args = args.concat(this.activeProject.cFiles)
      args.push('-o')
      args.push(path.join(this.activeProject.outputDir, this.activeProject.binName + (buildAssembly ? '.bc' : '')))
    }
  ...
  onRunPatchedBinary({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      command = path.join(this.activeProject.outputDir, 'instrumented_' + this.activeProject.binName)
      args = []
      // create a new process
      this.runningProcess = childProcess.spawn(command, {cwd: this.activeProject.outputDir})
    }
  ...
  onRunBinary({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      if ( !fs.existsSync(binary) ) {
        this.viewMgr.showNoCBinaryToExecuteNotification()
        return
      }
      // create a new process
      this.runningProcess = childProcess.spawn(binary)
    }
  ...
  onRunTeSSLa({onSuccess, onError}) {
    ...
    if (!atom.config.get('tessla.useDocker')) {
      ...
      if ( !fs.existsSync(server) ) {
        this.viewMgr.showNoTeSSLaServerFoundNotification()
        return
      }
    }
