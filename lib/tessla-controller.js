...
export default class TeSSLaController {
  ...
  onBuildTeSSLa({onSuccess, onError}) {
    ...
    this.runningProcess.on('close', (code, signal) => {
      ...
      if (stdout.charAt(0) == '{') {
        ...
        // first log message
        this.emitter.emit('add-log-text', {label: 'MSG', text: 'Successfully compiled ' + path.relative(this.activeProject.projPath, file).replace(/\\/g, '/')})
        ...
        // first log message
        this.emitter.emit('add-log-text', {label: 'MSG', text: 'An error occurred while compiling ' + file})
  ...
  checkDocker() {
    ...
    if ( id.length === '' ) {
      ...
      this.emitter.emit('add-log-text', {lael: 'CMD', text: 'docker ' + args.join(' ')})
    }
  }
