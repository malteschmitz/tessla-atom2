...
export default class TeSSLaController {
  ...
  onSetUpSplitView(firstSetUp) {
    ...
    if (activeEditor) {
      ...
      this.outputDir   = path.join(this.projectPath, 'build')
      this.binaryName  = path.basename(this.projectPath)
  ...
  onFileChanged(file) {
    ...
    if ( newProjectPath != this.projectPath ) {
      ...
      this.projectPath = newProjectPath
      this.outputDir   = path.join(this.projectPath, 'build')
      this.binaryName  = path.basename(newProjectPath)
  ...
  onCompileAndRunProject() {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      var traceFile = path.join(this.outputDir, 'instrumented_' + this.binaryName + '.trace')
    ...
    if ( fs.existsSync(path.join(this.outputDir, 'zlog.conf')) ) {
      fs.unlinkSync(path.join(this.outputDir, 'zlog.conf'))
    }

    // then create new zlog.conf file
    fs.writeFileSync(path.join(this.outputDir, 'zlog.conf'), formats + rules)
  ...
  onPatchAssembly({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      args = [
        ...
        path.join(this.outputDir, this.binaryName + '.bc')
      ]
      args = args.concat(instrumentArgs)
      args.push('>')
      args.push(path.join(this.outputDir, 'instrumented_' + this.binaryName + '.bc'))
  ...
  onBuildTeSSLa({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      args = [
        ...
        'cd /tessla && java -jar /tessla-imdea-snapshot.jar ' +
          path.relative(this.projectPath, file).replace(/\\/g, '/')
      ]
    } else {
      command = 'java'
    ...
    this.runningProcess.on('close', (code, signal) => {
      ...
      if (stdout.charAt(0) == '{') {
        ...
        require('fs').writeFileSync(path.join(this.outputDir, 'instrumented_' + this.binaryName + '.tessla.json'), stdout)
  ...
  onBuildAssembly({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      command = clang
      args = [
        path.join(this.outputDir, 'instrumented_' + this.binaryName + '.bc'),
        '-o',
        path.join(this.outputDir, 'instrumented_' + this.binaryName),
        '-lzlog',
        '-lpthread',
        '-L/usr/local/lib',
        '-L' + libDir,
  ...
  onBuildCCode({buildAssembly, onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      clangCommand += files.map((arg) => {
        return path.relative(this.projectPath, arg).replace(/\\/g, '/')
      }).join(' ')
      ...
      args.push('-o')
      args.push(path.join(this.outputDir, this.binaryName + (buildAssembly ? '.bc' : '')))
    }
  ...
  onRunPatchedBinary({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      command = path.join(this.outputDir, 'instrumented_' + this.binaryName)
      args = []
      // create a new process
  ...
  onRunBinary({onSuccess, onError}) {
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      // get the binary which will be executed
      var binary = path.join(this.outputDir, this.binaryName)
      // create a new process
    ...
    if (atom.config.get('tessla.useDocker')) {
      ...
      args = [
        ...
        'cd /tessla && LANG=C.UTF-8 /tessla_server ' +
          path.relative(this.projectPath, tsslJSON).replace(/\\/g, '/') + 
  ...
  setUpProjectStructure() {
    ...
    if (!fs.existsSync(path.join(this.projectPath, '.gcc-flags.json'))) {
      // spawn process
      ...
      which.on('close', (code) => {
        ...
        if (path.length > 0) {
          ...
          require('fs').writeFileSync(path.join(this.projectPath, '.gcc-flags.json'), fileContent)
        }
      })
    }
  }
