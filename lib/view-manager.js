...
export default class ViewManager {
  ...
  addIconsToTabs() {
    // add classes to tabs
    // document.querySelector('.tab[data-type=LogView]').classList.add('logViewTab');
    const messageViewTabs = document.querySelectorAll('.tab[data-type=MessageView]');
    const logViewTabs = document.querySelectorAll('.tab[data-type=LogView]');

    logViewTabs.forEach(tab => {
      if (tab.children[0].innerHTML === 'Log') {
        tab.classList.add('logViewTab');
      }
    });

    messageViewTabs.forEach(tab => {
      switch (tab.children[0].innerHTML) {
        case 'Warnings': tab.classList.add('warningsTab'); break;
        case 'Errors (TeSSLa)': tab.classList.add('errorsTeSSLaTab'); break;
        case 'Errors (C)': tab.classList.add('errorsCTab'); break;
        case 'Console': tab.classList.add('consoleTab'); break;
      }
    });
  }

  restoreViews() {
    console.log("restore views");
    ...
    Promise.all(viewSetup).then(views => {
      ...
      // reset objects in the views object
      for (const key in viewsContainer) {
        if (!(key in this.views)) {
          this.views[key] = viewsContainer[key];
        } else if (!this.views.key) {
          this.views[key] = viewsContainer[key];
        }
      }

      // add tab icons to them
      this.addIconsToTabs();
